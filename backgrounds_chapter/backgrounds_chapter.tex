...
\hline
%\multirow{2}{*}{Requirement} & \multicolumn{5}{c|}{Enriched background process} \\
%%\hhline{|~|-----|}
 %& \multicolumn{2}{c|}{$Z \to \mu^{+} \mu^{-}$} & \multirow{2}{*}{$W$ + jets} &
 %\multirow{2}{*}{$t\bar{t}$ + jets} & \multirow{2}{*}{QCD} \\
%%\hhline{|~|--|~~~|}
 %& Muon fake & Jet fake & & & \\
& \multicolumn{5}{c|}{Enriched background process} \\
%\hhline{|~|-----|}
 & \multicolumn{2}{c|}{$Z \to \mu^{+} \mu^{-}$} & & & \\
%\hhline{|~|--|~~~|}
Requirement & Muon fake & Jet fake & \WpJets & \ttbarpJets & QCD \\
\hline
\hline
...
\hline
\end{tabular}
\footnotetext[1]{``Very loose'' working point passed and ``Loose'' working point
failed}
\footnotetext[2]{No jets of $E_{T} > 20$~\GeV within $\vert \eta \vert < 2.1$ (other than the $\tau$ candidate)} 
\footnotetext[3]{Minimum two jets of $E_{T} > 40~\GeV$, at least one of which
has $E_{T} > 60$~\GeV and at least one with ``TrackCountingHighEff'' $b$--jet discriminator $> 2.5$}
...
regions]{\captiontext 
	 Distribution of visible mass of the muon plus the tau candidate reconstructed
         ...
         \ZMM~\subref{fig:becrZmumuMu_mVisible} and
         \subref{fig:becrZmumuJet_mVisible},
         \WpJets~\subref{fig:becrWpJ_mVisible},
         \ttbarpJets~\subref{fig:becrTTpJ_mVisible} and QCD
         multijet~\subref{fig:becrQCD_mVisible} backgrounds.
...
%         $t\bar{t}$ + jets (e) and QCD multi-jet (f) backgrounds.
         In \subref{fig:becrZmumuMu_mVisible} reconstructed tau candidates are expected to be dominantly due to misidentified muons,
         while in \subref{fig:becrZmumuJet_mVisible} they are expected to be mostly due to misidentified misidentified quark or gluon jets.}
...
\begin{center}
  ...
  \label{fig:becrQCD_mSVmethod}
  }
\caption[SVfit mass distribution of the backgrounds in the signal and control
regions]{\captiontext 
	 Distribution of the SVfit mass reconstructed
         in the background--enriched control samples for 
         \ZMM~\subref{fig:becrZmumuMu_mSVmethod} and
         \subref{fig:becrZmumuJet_mSVmethod},
         \WpJets~\subref{fig:becrWpJ_mSVmethod},
         \ttbarpJets~\subref{fig:becrTTpJ_mSVmethod} and QCD
         multijet~\subref{fig:becrQCD_mSVmethod} backgrounds.
%         $Z \to \mu^{+} \mu^{-}$ (a) and (b), $W$ + jets before (c) 
%         and after (d) the bias correction described in~cite{CMS_AN_2010-088}
%         $t\bar{t}$ + jets (e) and QCD multi-jet (f) backgrounds.
         In \subref{fig:becrZmumuMu_mSVmethod} reconstructed tau candidates are expected to be dominantly due to misidentified muons,
         while in \subref{fig:becrZmumuJet_mSVmethod} they are expected to be mostly due to misidentified misidentified quark or gluon jets.}
...
We take the average value as the ``best'' estimate of the background contribution
...
%
%The ``simple'' method described in the last section does actually overestimate
%the sum of background contributions to the final analysis by some amount.  The
%reason is that signal events present in the background dominated control sample
%(selected by not applying any tau identification cuts or discriminators)
%contribute to the estimate.  One way of handling the signal contribution is to
%subtract from the estimate obtained from the ``simple'' method a correction
%term which is to be estimated from Monte Carlo.  Another way is to correct for
%the signal contribution by adjusting the weights, based on information that is
%in the analyzed data sample only, avoiding the need to rely on Monte Carlo
%based corrections.  The idea of adjusting the weights in such a way that signal
%contributions cancel while an unbiased estimate of the sum of background
%contributions is maintained is the basis of the ``CDF--type'' fake--rate
%method.
%
...
To be specific, the ``CDF--type'' weights assigned to tau candidates are
computed as:
\begin{equation}
...
\mu^{+} \mu^{-}$ \subref{fig:visMassTemplZmumuMuon} and
\subref{fig:visMassTemplZmumuJet}, \WpJets before \subref{fig:visMassTemplWpJ}
and after \subref{fig:visMassTemplWpJCorr} the bias
correction explained in Section~\ref{sec:template}, \ttbarpJets
\subref{fig:visMassTemplTTpJets} and
QCD multi--jet \subref{fig:visMassTemplQCD} backgrounds enriched control regions compared to the expected
distribution of the of the enriched background process to the signal region,
predicted by Monte Carlo simulations.  In \subref{fig:visMassTemplZmumuMuon} reconstructed tau candidates
are expected to be dominantly due to misidentified muons, while in
\subref{fig:visMassTemplZmumuJet} they are
