...
The dominant background in the search for the Higgs $\to \TT$
signal is due to Standard Model $Z \to \tau^{+} \tau^{-}$ events.  The most
``natural'' observable to discriminate between Higgs signal and $Z$ background
would be the invariant mass of the di--tau system, utilizing the fact that the
$Z$ resonance is well known ($m_{Z} = 91.1876 \pm 0.0021$~\GeVcc) and has a narrow
width ($\Gamma_{Z} = 2.4952 \pm 0.0023$~\GeV)~\cite{PDG}.  The experimental
complication in this approach is due to the neutrinos produced in the tau lepton
decays, which escape detection, carrying away an unmeasured amount of energy,
and making it difficult to reconstruct the tau lepton four--vectors.
...
separate the Higgs signal from the $Z \to \TT$ background.
...
\item matching between the momenta of neutrinos produced in the tau decays
 and the reconstructed missing transverse momentum
...
\item the compatibility of tau decay parameters with the position of reconstructed tracks
 and the known tau lifetime of $c \cdot \tau = 87~\micron$~\cite{PDG}.
\end{itemize}
...
taken to be a Gamma distribution of scale parameter $\theta$ and shape parameter
k, in order to account for tails in the $P_{T}$ distribution of the tau lepton
pair.  The complete functional form is thus given by
\begin{equation}
...
\caption{Comparison of the reconstructed tau pair mass spectrum in \ZTT~(left)
and MSSM \HTT{120}~(right) events after the selections described in
