...
\label{ch:svfit}
%
The dominant background in the search for a Higgs boson decaying to a $\TT$ pair are 
standard model $\ZTT$ events.  The most ``natural'' observable to discriminate
between a Higgs boson signal and the $Z$ background is the invariant mass of the
...
distribution in simulated $H \to\TT$ events.  The relative weight a of the
...
provides the observable with the largest separation between signal Higgs boson events
