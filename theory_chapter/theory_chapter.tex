...
discovery the Higgs.  In this section we discuss how the Higgs and MSSM) could
...
Standard Model and MSSM Higgs.  Additionally, precision electroweak measurements
...
the prediction of the Standard Model.  For Higgs masses below $114~\GeVcc$, the
...
significant.  Low di--boson backgrounds allow this decay mode to probe both
...
terms of the MSSM\@.  To address the pair production mode, searches were performed
in the $e^+e^- \to h^0 A^0 \to \bbbar \bbbar$ and $\TT \qqbar$ decay modes.
Finally, LEP is also sensitive to at low $\ma$ and high $\tan\beta$ to
associated production, $\EE \to \mathcal{f}\overline{\mathcal{f}} \phi$, where
the associated fermions $\mathcal{f}$ are \mbox{$b$--quarks} or tau leptons.
The combined limits from LEP in the $\ma--\tan\beta$ plane are shown in
...
The lifetime of the tau is $c\tau = 87~\micro\meter$.  In practice, this means
that a tau with produced with energy $E$ travels on average 
\begin{equation}
...
it may be possible to use it to reconstruct the associated neutrinos.
