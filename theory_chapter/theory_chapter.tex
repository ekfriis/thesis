...
QED is a gauge theory, which means that the physical observables are
...
\end{equation}
The Lagrangian is invariant under local gauge translations if $\theta$
...
required if the Lagrangian is to satisfy local gauge invariance. The
...
a contact theory, all four fermions involved in the $\beta$--decay are
connected at a single vertex.  The Fermi theory Hamiltonian for the
$\beta$--decay of a neutron is then~\cite{Morii:SMandBSM}
\begin{equation}
...
Furthermore, the coupling constant $G$ is found to be a universal
...
\begin{figure}
  ...
  \begin{fmffile}{muon_decay}
    ...
    \end{fmfgraph*}
  \end{fmffile}
  \caption[Fermi contact interaction diagram]{Feynman diagram of muon decay
...
\begin{equation}
  \biggl(\begin{array}{c} \phi_1 \\ \phi_2 \end{array}\biggr)
    \to
  \biggl(\begin{array}{c} \phi'_1 \\ \phi'_2 \end{array}\biggr)
    =
  \biggl(\begin{array}{cc} \cos\theta & -\sin\theta \\ 
    \sin\theta & \cos\theta \end{array}\biggr)
  \biggl(\begin{array}{c} \phi_1 \\ \phi_2 \end{array}\biggr),
    \nonumber
...
\begin{eqnarray}
  ...
  \sum_{a=1,2} \biggl(\frac {\partial V}{\partial \phi_a}\biggr)_{0} (\phi_a -
  \phi^{min}_a) \nonumber \\
  &+ &\frac{1}{2} \sum_{a,b=1,2} \biggl(\frac {\partial^2 V}{\partial \phi_a \partial
  \phi_b}\biggr)_{0} (\phi_a - \phi^{min}_a)(\phi_b - \phi^{min}_b) + \ldots
...
V}{\partial \phi_a \partial \phi_b}\biggr)_{\phi^{min}}
...
is at the origin \mbox{$(\phi_1 = \phi_2 = 0)$}.  In this case, the mass matrix term in
Equation~\ref{eq:ExpandedPotential} takes the form \mbox{$\biggl(\frac {\partial^2
V}{\partial \phi_a \partial \phi_b}\biggr)_{0} = \frac{m^2}{2}\delta_{ab}$},
...
requirement to be locally invariant creates interesting consequences for
...
a massive scalar $\eta$ with $m = \sqrt{2\mu^2}$ and a massive gauge
...
\begin{equation}
  L = \frac{1 - \gamma_5}{2}
  \biggl(\begin{array}{c} \nu_e \\ e^- \end{array}\biggr)
    = \biggl(\begin{array}{c} \nu_e \\ e^- \end{array}\biggr)_L
      \label{eq:EWDoubletForm}
\end{equation}
The operators that operate on ``weak isospin,'' the quantum of $SU(2)_L$,
are \begin{eqnarray}
  \tau^+ &=& \frac{\tau^1 + i \tau^2}{2} = 
  \biggl(\begin{array}{cc} 0 & 1 \\ 0 & 0 \end{array}\biggr) 
    %\label{eq:Su2GeneratorPlus} 
    \nonumber \\
  \tau^- &=& \frac{\tau^1 - i \tau^2}{2} = 
  \biggl(\begin{array}{cc} 0 & 0 \\ 1 & 0 \end{array}\biggr),
...
\begin{eqnarray}
  ...
  \biggl(\begin{array}{cc} 1 & 0 \\ 0 & -1 \end{array}\biggr) L \nonumber \\
...
\begin{eqnarray}
  \phi &=& \biggl(\begin{array}{c}\phi^+ \\ \phi^0\end{array}\biggr) \nonumber \\
...
\begin{equation}
  \phi_{min} = \biggl(\begin{array}{c} 0 \\ v/\sqrt{2} \end{array}\biggr).
    \nonumber
\end{equation}
...
and $Y$ are all broken.  The vacuum is invariant under $Q$, the generator
of $U(1)_{em}$:
\begin{equation}
  Q\phi_{min} = (T^3 + \frac{Y}{2})\biggl(\begin{array}{c} 0 \\ v/\sqrt{2}
  \end{array}\biggr) = 0 \nonumber,
\end{equation}
...
\begin{equation}
  \phi' =  \biggl(\begin{array}{c} 0 \\ \frac{1}{\sqrt 2}(\nu +
    H(x))\end{array}\biggr) = 
    \frac{1}{\sqrt 2}(\nu + H(x))\chi.
...
invariant under color transformations.  Color is the charge of QCD and
...
diagram, additional gluon interactions contribute a value greater than the
previous order of calculation.  The
dominance of multi--loop diagrams is illustrated in
Figure~\ref{fig:QCDFeynmanDiagrams}.
\begin{figure}
  \centering
  \makebox[0.7\textwidth]{\rule[3pt]{0pt}{6mm}}
...
\begin{figure}
  \centering
  \makebox[0.7\textwidth]{\rule[3pt]{0pt}{4mm}}
...
have the same value, but opposite sign.  Thus the large $\Lambda^2$
...
SM is the MSSM\@.  The model groups
...
right--handed gaugino fermion) ``supermultiplets.''  The superpartner of each
particle is responsible for cancelling the loop corrections divergences to the
Higgs mass. 
As the weak
...
supermultiplets defined in Table~\ref{tab:chiral}.  The ${\bf y}$ terms are Yukawa
...
biggest prizes in science today.  Many experiments, thousands of scientists
...
\end{equation}
\fixme{Is this a 2 in~\ref{eq:HiggsVEVtoCoupling}?}
...
mode is ``Higgstrahlung,'' where an off--shell $W^\pm$ or $Z$ gauge boson is
...
CDF and D\O. The dominant Higgs boson production modes at the Tevatron are
...
significant.  Low diboson backgrounds allow this decay mode to probe both the
...
low $\ma$ and high $\tan\beta$ to $\EE \to f\overline f \phi$,
where the associated fermions $f$ are \mbox{$b$--quarks} or tau leptons.
...
scenarios is shown in Figure~\ref{fig:TevMSSMLimits}.
\begin{figure}
  \centering
...
thesis will describe a novel way to reconstruct the neutrinos associated with tau
decays in Chapter~\ref{ch:svfit}.  

A tau produced with energy $E$ travels on average 
\begin{equation}
